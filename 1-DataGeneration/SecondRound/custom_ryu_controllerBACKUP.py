...
class CustomRyuController(app_manager.RyuApp):
    ...
    def create_feature_array(self, features):
        try:
            ...
            feature_array = np.array([
                int(features.get('eth.src', 0).replace(':', ''), 16),
                int(features.get('eth.dst', 0).replace(':', ''), 16),
                struct.unpack("!L", socket.inet_aton(ip_src))[0],
                struct.unpack("!L", socket.inet_aton(ip_dst))[0],
                features.get('frame.len', 0),
                self.protocol_encoder.fit_transform(
                    [features.get('_ws.col.Protocol', 'Unknown')])[0],
                features.get('frame.time.delta', 0),
    ...
    def update_connection_stats(self, connection_id, packet_size, current_time):
        if connection_id not in self.connections:
            ...
            self.connections[connection_id]=[current_time, conn_packets, conn_bytes]
    ...
    def write_to_csv(self, file_name, eth_src, eth_dst, ip_src, ip_dst, frame_len, protocol, frame_time_delta, identical_packets_per_second, bytes_per_second, unique_packets_per_second, packet_arrival_rate, probabilities):
        file_exists=os.path.isfile(file_name)
        header=['eth.src', 'eth.dst', 'ip.src', 'ip.dst', 'frame.len', '_ws.col.Protocol', 'frame_time_delta',
                  'identical_packets_per_second', 'bytes_per_second', 'unique_packets_per_second', 'packet_arrival_rate', 'probabilities']
        row=[eth_src, eth_dst, ip_src, ip_dst, frame_len, protocol, frame_time_delta,
               identical_packets_per_second, bytes_per_second, unique_packets_per_second, packet_arrival_rate, probabilities]
    ...
    def packet_in_handler(self, ev):
        ...
        feature_array=self.create_feature_array({
            ...
            'frame.time.delta': frame_time_delta,
        ...
        self.write_to_csv('badaaaa3_network_data.csv', eth.src, eth.dst, src_ip, dst_ip, frame_len, proto, frame_time_delta,
                          identical_packets_per_second, bytes_per_second, unique_packets_per_second, packet_arrival_rate, probabilities)
        # probabilities = self.model.predict_proba(feature_array)
