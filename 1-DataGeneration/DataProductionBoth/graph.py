...
data1 = pd.read_csv('preprocessed_data_FirstRound.csv')

# Read the second CSV file
data2 = pd.read_csv('preprocessed_data_SecondRound.csv')
...
# Plotting the graph
plt.plot(x_coordinates_1, packet_arrival_rate_1, label='Packet Arrival Rate (First)', color='red')
plt.plot(x_coordinates_2, packet_arrival_rate_2, label='Packet Arrival Rate (Second)', color='blue')
plt.xlabel(f'Time ({time_unit_1}/{time_unit_2})')
plt.ylabel('Packet Arrival Rate')
plt.title('Combined Graph: Packet Arrival Rate')
plt.legend()
plt.grid(True)
plt.show()
